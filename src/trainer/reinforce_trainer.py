...
class ReinforceTrainer:
    ...
    def __init__(self, cfg: TrainConfig):
        ...
        if cfg.multi_gpu == "none":
            ...
            # When using MindFace with disjoint-subset FSDP, we do NOT let
            # Accelerate wrap the model, as we manually construct FSDP groups.
            if getattr(cfg, "use_mind_face", False):
                self.accelerator = Accelerator()
                print("[Trainer/__init__] Using custom FSDP (MindFace) flow; Accelerator without FSDP plugin.", flush=True)
            else:
        ...
        if getattr(cfg, "use_mind_face", False):
            ...
            # FSDP disjoint-subset path: build per-role model and groups
            if cfg.multi_gpu == "fsdp":
                if not (dist.is_available() and dist.is_initialized()):
                    raise RuntimeError("Distributed must be initialized for FSDP MindFace")
                world_size = dist.get_world_size()
                if world_size < 2:
                    raise ValueError("FSDP MindFace requires at least 2 GPUs")
                mind_size = (world_size + 1) // 2
                face_size = world_size - mind_size
                if mind_size == 0 or face_size == 0:
                    raise ValueError("Invalid rank split for FSDP MindFace")

                mind_ranks = list(range(mind_size))
                face_ranks = list(range(mind_size, world_size))
                mind_group = dist.new_group(mind_ranks)
                face_group = dist.new_group(face_ranks)

                rank = dist.get_rank()
                fsdp_role = "mind" if rank in mind_ranks else "face"
                print(f"[Trainer/__init__][rank{rank}/{world_size}] role={fsdp_role} | mind_ranks={mind_ranks} face_ranks={face_ranks}", flush=True)
                ...
                self.model = MindFace(
                    ...
                    fsdp_mind_group=mind_group,
                    fsdp_face_group=face_group,
                    fsdp_role=fsdp_role,
                )
            else:
                self.model = MindFace(
        ...
        # In MindFace+FSDP disjoint-subset mode, the model is already FSDP-wrapped
        # and must not be re-wrapped by Accelerate.
    ...
    def _generate_sequences(self, inputs, gen_kwargs):
        ...
        Returns a tuple: (sequences, meta) where meta may include
        'prompt_lens' when using MindFace FSDP mode.
        """
        ...
        if isinstance(model_for_check, MindFace):
            ...
            meta = {}
            if hasattr(generated, "prompt_lens"):
                meta["prompt_lens"] = generated.prompt_lens
