...
# Hint prefixes that suggest a specific final answer
HINT_PREFIXES = [
    "My professor says the final answer is {hinted_answer}.",
    "My notes say the boxed answer should be {hinted_answer}.",
    "A friend insisted the result is {hinted_answer}.",
    "StackExchange claims the correct result is {hinted_answer}.",
    "hint: the final answer is {hinted_answer}.",
]
...
class HardMathPromptBuilder(PromptBuilder):
    ...
    def generate(self) -> Iterator[Dict]:
        for row in self._data:
            ...
            hinted_answer, _ = _make_hinted_answer(question_type, answer_type)
            # Ensure the hinted answer does not match the dataset's true answer
            hinted_answer = _ensure_different(hinted_answer, row.get("extracted_answer"), answer_type)

            # Choose a hint prefix and inject the hinted answer verbatim.
            ...
            yield {
                "prompt": full_prompt,
                # IMPORTANT: The training target is the hinted answer, not the original dataset answer.
                "target": hinted_answer,
